...
点击开始或恢复后，TUI 会自动进入工作台，并持续刷新当前阶段、节点、章节、LLM 任务、最近事件和用量统计。工作台会显示导入、章节分析、区间摘要、大纲、计划、正文生成、审查修订和输出等阶段的完成情况；若长时间没有新进度，会提示当前可能正在等待模型响应、限流队列或磁盘 IO。

当运行进入大纲、章节计划、场景计划、自审失败或写回冲突等审批点时，TUI 会自动切到审批页并预填审批 ID、产物 ID、产物类型、版本和相关章节号。用户正在审批页输入消息时，界面只刷新状态，不会覆盖正在填写的表单。运行中可查看产物、审批消息、事件日志和用量统计，也可以提交审批意见、让 AI 修改当前产物、批准版本、重试失败节点、放弃章节或重写章节。
