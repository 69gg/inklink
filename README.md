...
- 运行 ID：留空时创建新运行；填写已有 ID 后可点“连接”查看仍在运行的任务，或点“续接”恢复已停止/已暂停的任务。页面会持久化运行设置和 notes，但不会在重启后自动连接旧 runtime。
- 续写章数与起始章节。
...
当运行进入大纲、章节计划、场景计划、自审失败或写回冲突等审批点时，右侧审批区会直接显示当前审批 ID、产物 ID、类型、版本和完整 JSON 内容预览。用户可以提交审批意见、让 AI 修改当前产物、批准版本、重试失败节点、放弃章节或重写章节。
