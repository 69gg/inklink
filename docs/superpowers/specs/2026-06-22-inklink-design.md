...
TUI 必须提供章节级控制操作，包括暂停、恢复、批准、重试、放弃章节和重写章节。`abandon_chapter` 用于放弃已生成或已集成但未落盘的章节，并触发相关 artifact、索引更新和下游节点失效。`rewrite_chapter` 用于从章节计划或场景计划重新生成该章节。
...
检索结果可配置显式 token 预算。该预算默认关闭；开启后组装 prompt 时按确定性优先级裁剪，不让命中材料无限增长。裁剪优先级：
