...
TUI 必须提供章节级控制操作，包括暂停、恢复、批准、重试、放弃章节和重写章节。`abandon_chapter` 用于放弃已生成或已集成但未落盘的章节，并触发相关 artifact、索引更新和下游节点失效。`rewrite_chapter` 用于从章节计划或场景计划重新生成该章节。放弃或重写章节时必须递增章节级 `generation` 或 `regeneration_count`，该值纳入后续 LLM 调用幂等键，确保不会复用刚被放弃的旧响应。
...
`active_score` 必须是底层提及事实的纯函数，而不是原地累加的运行态计数器。索引层需要保留每章节、每实体的提及事实记录，例如“人物 X 在第 N 章被提及，强度 Y”。分数由这些事实和衰减规则计算得出，保证并行、重试、恢复或重复合并同一章节分析时结果一致。
...
- 如果用户放弃某个已集成但未落盘的生成章节，必须通过显式操作使该章节的 artifact、索引更新和下游节点失效。
...
- 活跃度测试：`active_score` 从提及事实重算，重复合并同一章节分析不改变结果。
...
- 索引更新必须与并行章节分析完成顺序无关。
- 列表型索引字段必须用集合语义合并，active_score 必须由提及事实重算。
